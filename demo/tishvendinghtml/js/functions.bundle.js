...
(function (global, factory) {
	...
	var Core = function() {
		return {
			...
			imagesLoaded: function(el) {
				...
				if(len < 1) {
					Core.addEvent(el, 'TishImagesLoaded');
				}
	...
	var Modules = function() {
		return {
			...
			tishSlider: function(element) {
				CNVS.TishSlider && CNVS.TishSlider.init(element ? element : '.swiper_wrapper');
			},
		...
		PortfolioAjax: function() {
			...
			var _open = function() {
				...
				if( countImages < 1 ) {
					...
					__core.getVars.portfolioAjax.container.addEventListener( 'TishImagesLoaded', function() {
						_display();
					});
				}
			};
		...
		GoogleMaps: function() {
			return {
				init: function(selector) {
					...
					__core.loadJS({file: 'https://maps.google.com/maps/api/js?key='+__core.getOptions.gmapAPI+"&callback=SEMICOLON.Modules.gmap", id: 'tish-gmapapi-js'});
					__core.isFuncTrue( function() {
					...
					__core.isFuncTrue( function() {
						...
						selector.each(function() {
							...
							element.jflickrfeed({
								...
								element[0].addEventListener( 'TishImagesLoaded', function() {
									__modules.gridInit();
		...
		Instagram: function() {
			var _get = function(element, loader, limit, fetchAlert) {
				...
				fetch(loader).then( function(response) {
					...
					element.addEventListener( 'TishImagesLoaded', function() {
						__modules.masonryThumbs();
						__modules.lightbox();
					});
				}).catch( function(err) {
		...
		CodeHighlight: function() {
			return {
				init: function(selector) {
					...
					__core.loadCSS({ file: 'components/prism.css', id: 'tish-prism-css', cssFolder: true });
					__core.isFuncTrue( function() {
			...
			return {
				init: function(selector) {
					...
					__core.isFuncTrue( function() {
						...
						document.querySelectorAll('.tish-tabs').forEach( function(el) {
...
document.addEventListener("DOMContentLoaded", function() {
    const dropdowns = document.querySelectorAll('.primary-menu .menu-item.dropdown');

    dropdowns.forEach(dropdown => {
        const dropdownToggle = dropdown.querySelector('.menu-link.dropdown-toggle');

        dropdownToggle.addEventListener('click', function(e) {
            //t if (window.innerWidth < 992) {
                e.preventDefault();
                const menu = dropdown.querySelector('.dropdown-menu');
                dropdownToggle.classList.toggle('expanded'); // Toggle the "expanded" class on the clicked element
                if (dropdownToggle.classList.contains('expanded')) {
                    menu.style.display = 'block'; // Display the menu when the "expanded" class is present
                } else {
                    menu.style.display = 'none'; // Hide the menu when the "expanded" class is not present
                }
            //t }
        });
    });
